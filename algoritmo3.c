...
static void mutacao_local(Individuo *ind, int m) {
...
double hibrido_refinamento_final(int *sol, int m, int C, int pop_size, int num_geracoes, int num_iter_tc) {
    algoritmo_evolutivo(sol, m, C, pop_size, num_geracoes);
    double fitness_final = trepa_colinas(sol, m, C, num_iter_tc);
...
double hibrido_memetico(int *sol, int m, int C, int pop_size, int num_geracoes) {
    ...
    for (int g = 0; g < num_geracoes; g++) {
        if (g % 10 == 0) {
            for(int i=0; i < pop_size; i++) {
                if(rand_01() < 0.1) {
                    populacao[i].fitness = trepa_colinas(populacao[i].genes, m, C, 50);
                }
            }
        }
        ...
        for (int i = 1; i < pop_size; i += 2) {
            ...
            p1.genes = malloc(sizeof(int)*m); p2.genes = malloc(sizeof(int)*m);
            f1.genes = malloc(sizeof(int)*m); f2.genes = malloc(sizeof(int)*m);
            ...
            if(rand_01() < 0.05) mutacao_local(&f1, m);
            if(rand_01() < 0.05) mutacao_local(&f2, m);
            ...
            free(p1.genes); free(p2.genes); free(f1.genes); free(f2.genes);
        }
